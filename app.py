...
def telegram_data():
    ...
    return {
        ...
        'hash_token': params.get("hash", ["None"])
    }
...
if str(user_info['hash_token']) in takeUserList():
    ...
    if 'balance' not in st.session_state:
        st.session_state.balance = takeBalance(user_info['hash_token'])
    ...
    # Блок баланса
    with st.container():
        st.markdown(f"<div style='text-align:center; font-size:1.1rem; margin:5px 0;'>@{user_info['name']}</div>", unsafe_allow_html=True)
    with st.container():
    ...
    # CSS для адаптации под мобильные устройства
    st.markdown(
        """
        <style>
            .main > div {
    ...
    # Текст ошибки
    st.markdown("<h1>⚠️ Ошибка</h1>", unsafe_allow_html=True)
    st.markdown(
        """
        <p>Произошла непредвиденная ошибка</p>
        <p>Сайт работает только с ботом</p>
        """, 
        unsafe_allow_html=True
    )
